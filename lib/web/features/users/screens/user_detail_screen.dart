...
class _UserDetailScreenState extends State<UserDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (user == null)
      return Center(child: Text(provider.error ?? l.userNotFound));
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      fullName.isEmpty ? l.userNoName : fullName,
                      style: const TextStyle(
                        fontSize: 22,
              ...
              return GridView.count(
                ...
                children: [
                  KpiCard(
                    ...
                    label: l.userReservations,
                    value: '${provider.userReservationCount}',
                  ),
                  KpiCard(
                    icon: Icons.attach_money,
                    label: l.userTotalSpend,
                    value: '\$${provider.userTotalSpend.toStringAsFixed(2)}',
                  ),
                  KpiCard(
                    icon: Icons.person_add,
                    label: l.userRegistered,
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _InfoRow(
                    label: l.userFirstName,
                    value: user.firstName ?? '—',
                  ),
                  _InfoRow(label: l.userLastName, value: user.lastName ?? '—'),
                  _InfoRow(
                    label: l.userDateOfBirth,
                    value: user.dateOfBirth?.toString().split(' ')[0] ?? '—',
                  ),
                  _InfoRow(
                    label: l.userBillingAddress,
                    value: user.billingAddress ?? '—',
                  ),
                  _InfoRow(
                    label: l.userPreferredLanguage,
                    value: user.preferredLanguage ?? 'es',
                  ),
                  _InfoRow(
                    label: l.userUpdated,
                    value: user.updatedAt.toString().split('.')[0],
                  ),
                ],
              ),
            ),
